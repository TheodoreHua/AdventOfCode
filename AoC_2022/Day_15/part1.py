...
def main(d: list, bar, row=2000000):
    ...
    nb_ranges = []  # List of ranges that contain the X values that are not beacons
    ...
    for i in d:
        ...
        nb_ranges.append(range(sensor_x - manhattan, sensor_x + manhattan + 1))  # range of X values that are not beacons
        bar()
    ...
    return len(range(min([i.start for i in nb_ranges]), max([i.stop for i in nb_ranges]))) - len(beacon_xs)
