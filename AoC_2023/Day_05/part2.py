...
def main(d: list, bar):
    seeds_raw = DIGIT_REGEX.findall(SEED_REGEX.fullmatch(d[0]).group(1))
    seeds = []
    for i in range(0, len(seeds_raw), 2):
        start = int(seeds_raw[i])
        seeds.append(range(start, start + int(seeds_raw[i+1])))
    maps = {}
    last_map = None
    for line in d:
    ...
    # results = []
    print(sum(len(seed_r) for seed_r in seeds))
    lowest = float('inf')
    for seed_r in seeds:
        for seed in seed_r:
            if (result := get_last(seed, maps)) < lowest:
                lowest = result
            bar()
