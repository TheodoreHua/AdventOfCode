...
def get_score_value(hand, val_hand=None):
    occur = Counter(hand)
    if "J" in hand and len(set(hand)) != 1:
        return min(get_score_value(hand.replace("J", i), val_hand=hand) for i in occur if i != "J")
    ...
    return ts + get_hand_score(hand if val_hand is None else val_hand)
