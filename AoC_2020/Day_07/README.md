...
food: all flights are currently delayed due to _issues in luggage processing_.
...
You have a `_shiny gold_` bag. If you wanted to carry it in at least one other bag, how many different bag colors would
...
_How many bag colors can eventually contain at least one `shiny gold` bag?_ (The list of rules is quite long; make sure
you get all of it.)
...
the 11 bags within _each_ of those): `1 + 1*7 + 2 + 2*11` = `_32_` bags!

Of course, the actual rules have a <span title="100%">small</span> chance of going several levels deeper than this
