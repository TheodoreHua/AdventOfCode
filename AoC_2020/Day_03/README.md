...
certainly not safe:
there's <span title="It looks like the toboggan steering system even runs on Intcode! Good thing you don't have to modify it.">
very minimal steering</span> and the area is covered in trees. You'll need to see which angles will take you near the
fewest trees.
...
by _counting all the trees_ you would encounter for the slope _right 3, down 1_:
...
The locations you'd check in the above example are marked here with `_O_` where there was an open square and `_X_` where
there was a tree:
...
Starting at the top-left corner of your map and following a slope of right 3 and down 1, _how many trees would you
encounter?_
...
these produce the answer `_336_`.
